...
void loop() {
  ...
  uint32_t SIDE1_END = 15;
  uint32_t SIDE2_START = 12;
  uint32_t SIDE2_END = 23;
  ...
  // master params
  uint32_t RUN = 0;
  uint32_t DELAY_SECONDS = 1;
  uint32_t DELAY_MS = DELAY_SECONDS * 1000;
  uint32_t BLANK_SECONDS = 1;
  ...
  if(RUN){
    ...
    // do side 2
    //activate_array(SIDE2_START, SIDE2_END, BLUE);
    //delay(DELAY_MS);
